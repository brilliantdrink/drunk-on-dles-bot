...
const pythonScriptPath = path.resolve(projectDir, 'src', 'py', 'worldle_get_guesses_structured.py')
const request = invokeWorker(pythonScriptPath)

export async function worldleGetGuessesStructured(image: Sharp) {
  ...
  const guessesStructured = await Promise.all(
    matrix.matrix.map(row => {
      ...
      return recogniseTextRects(image.clone(), row as unknown as Region[])
    })
  )
